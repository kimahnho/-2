...
export const TEMPLATES = [
    ...
    },
    {
        ...
        thumbnail: '',
        elements: (() => {
            const elements: Partial<DesignElement>[] = [];
            const gridSize = 4;
            const cardSize = 140;
            const gap = 15;
            const startX = 60;
            ...
            // Title
            elements.push({
                type: 'text', x: 50, y: 50, width: 300, height: 40,
                content: 'AAC 의사소통 판', fontSize: 28, color: '#5500FF', rotation: 0,
            ...
            for (let row = 0; row < gridSize; row++) {
                for (let col = 0; col < gridSize; col++) {
                    ...
                    // Card background
                    elements.push({
                        type: 'card', x, y, width: cardSize, height: cardSize,
                        backgroundColor: '#ffffff', borderRadius: 16,
                        borderWidth: 3, borderColor: '#E5E7EB', borderStyle: 'solid',
                        rotation: 0
                    });

                    // Placeholder text
                    elements.push({
                        type: 'text', x: x + 10, y: y + cardSize / 2 - 15, width: cardSize - 20, height: 30,
                        content: '카드 추가', fontSize: 16, color: '#9CA3AF', rotation: 0,
                        fontFamily: "'Gowun Dodum', sans-serif"
                    });
                }
            }

            // Sentence composition area at bottom
            const sentenceY = startY + gridSize * (cardSize + gap) + 20;
            elements.push({
                type: 'shape', x: startX, y: sentenceY, width: gridSize * (cardSize + gap) - gap, height: 80,
                backgroundColor: '#F3E8FF', borderRadius: 16, rotation: 0
            });
            elements.push({
                type: 'text', x: startX + 20, y: sentenceY + 25, width: 200, height: 30,
                content: '문장 구성 영역', fontSize: 18, color: '#7C3AED', rotation: 0,
                fontFamily: "'Gowun Dodum', sans-serif"
            });
        ...
        elements: (() => {
            ...
            for (let row = 0; row < gridSize; row++) {
                for (let col = 0; col < gridSize; col++) {
                    ...
                    elements.push({
                        type: 'text', x: x + 10, y: y + cardSize / 2 - 15, width: cardSize - 20, height: 30,
                        content: '카드 추가', fontSize: 18, color: '#9CA3AF', rotation: 0,
                        fontFamily: "'Gowun Dodum', sans-serif"
                    });
                }
            }
