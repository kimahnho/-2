...
export const PageManager: React.FC<Props> = ({
  ...
  return (
    <div className="h-[200px] bg-white border-t border-gray-200 flex flex-col shrink-0 z-10 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)] no-print transition-all">
      ...
      <div className="flex-1 overflow-x-auto overflow-y-hidden flex items-start px-4 custom-scrollbar bg-gray-50/30 pt-4">
        {pages.map((page, index) => {
          ...
          return (
            <React.Fragment key={page.id}>
              ...
              {/* Insertion Trigger (between pages) */}
              <div
                className="self-stretch flex items-center justify-center relative group cursor-pointer w-2 hover:w-12 transition-all duration-300 ease-in-out"
                title="페이지 삽입"
                onClick={() => onAddPage('portrait', index + 1)}
              >
                <div className="absolute w-0.5 h-1/2 bg-[#5500FF] opacity-0 group-hover:opacity-100 transition-opacity delay-100" />
                <div className="w-6 h-6 rounded-full bg-[#5500FF] text-white flex items-center justify-center opacity-0 group-hover:opacity-100 shadow-md transform scale-0 group-hover:scale-100 transition-all duration-300 z-20 whitespace-nowrap">
