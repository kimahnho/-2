...
export const PageManager: React.FC<Props> = ({
  ...
  return (
    <div className="h-[200px] bg-white border-t border-gray-200 flex flex-col shrink-0 z-30 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)] no-print transition-all">
        ...
        {pages.map((page, index) => {
          ...
          return (
            <div 
              ...
              className={`page-thumbnail group relative flex-shrink-0 flex flex-col items-center gap-2 transition-all duration-200 outline-none ${
                isActive ? 'opacity-100' : 'opacity-70 hover:opacity-100'
              }`}
            >
              {/* Thumbnail Container */}
              <div 
                className={`relative bg-white rounded-md overflow-hidden cursor-pointer transition-all ${
                  isActive 
              ...
              >
                ...
                {/* Page Number Badge */}
                <div className={`absolute bottom-1 right-1 text-[9px] px-1.5 py-0.5 rounded-full backdrop-blur-sm font-medium ${
                    isActive ? 'bg-[#5500FF] text-white' : 'bg-black/40 text-white'
                }`}>
                  {index + 1}
                </div>
