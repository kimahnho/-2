...
export const ProjectList: React.FC<Props> = ({
    ...
    const handleDelete = (e: React.MouseEvent, id: string) => {
        ...
        e.preventDefault();
        setActiveMenuId(null);
        // Small delay to let menu close before confirm appears
        setTimeout(() => {
            onDeleteProject(id);
        }, 100);
    };
    ...
    return (
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6">
            ...
            {projects.map(project => (
                ...
                >
                    {/* Thumbnail Area */}
                    <div className="flex-1 bg-gray-50 relative overflow-hidden flex items-center justify-center rounded-t-xl group-hover:bg-gray-100 transition-colors">
                        {project.thumbnail ? (
                            <img src={project.thumbnail} className="w-full h-full object-cover" alt="Preview" />
                        ) : project.previewElements && project.previewElements.length > 0 ? (
                            <div
                                className="relative bg-white shadow-sm transition-transform duration-300 group-hover:scale-105"
                                style={{
                                    width: 200, // Approx A4 width * 0.25
                                    height: 280, // Approx A4 height * 0.25
                                    overflow: 'hidden'
                                }}
                            >
                                <div style={{
                                    ...
                                    {project.previewElements.map((el: any) => (
