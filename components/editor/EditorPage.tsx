...
export const EditorPage: React.FC<Props> = ({ projectId, initialData, initialTitle, onBack }) => {
  // --- 1. Domain State (Data) ---
  ...
  }, [project.elements, project.pages, title, projectId]);
  ...
  return (
    <div className={`flex h-screen bg-gray-100 overflow-hidden font-sans select-none ${viewport.isPanning ? 'cursor-grabbing' : ''}`}
      ...
      <PropertiesPanel 
        ...
        onAlign={project.alignSelected} onGenerateImage={actions.handleAiImageFill}
      />
    </div>
  );
};
